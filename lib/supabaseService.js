 ...
 * Fixed version with improved upsert logic for sales_metrics table
 */
...
class SupabaseService {
   ...
   * Fixed to handle duplicates and proper granularity mapping
   */
  async upsertSalesData(salesData, source = 'sp-api') {
    try {
      ...
      // Transform data to match exact sales_metrics schema
      const transformedData = uniqueData.map(record => ({
        ...
        granularity: 'Day', // Fixed granularity
        source: source,
      ...
      const { data, error } = await this.supabase
        ...
        .upsert(transformedData, { 
          onConflict: 'date,marketplace_id,granularity', // Complete compound unique key
  ...
  async upsertIndividualSalesData(salesData, source = 'sp-api') {
    try {
      ...
      salesData.forEach(record => {
        ...
        for (let i = 0; i < unitsOrdered && i < 10; i++) {
