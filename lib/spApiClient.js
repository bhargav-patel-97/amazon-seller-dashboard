...
class SPAPIClient {
   ...
   * Fixed to use proper SP-API interval format and granularity mapping
   */
  async getSalesData(startDate, endDate, granularity = 'Daily') {
    ...
    try {
      ...
      // Map granularity to SP-API expected values
  ...
  /**
   * Map granularity values to SP-API expected format
   * @param {string} granularity - Input granularity (Daily, Hourly, Weekly, etc.)
   * @returns {string} SP-API compatible granularity
   */
  mapGranularityToSPAPI(granularity) {
    const granularityMap = {
      ...
      'Weekly': 'Week',
      'Monthly': 'Month',
      // Handle direct SP-API values as well
      'Day': 'Day',
      'Hour': 'Hour',
      'Week': 'Week',
      'Month': 'Month'
    ...
    return granularityMap[granularity] || 'Day'; // Default to Day if unknown
  }
  ...
  toISO8601WithTimezone(dateString, endOfDay = false) {
    ...
    if (endOfDay) {
      // For end date, set to end of day inclusive in the marketplace timezone
  ...
  /**
   * Get increment days based on granularity
  ...
  getIncrementDays(granularity) {
    const incrementMap = {
      'Daily': 1,
      'Day': 1,
      'Hourly': 1, // For hourly, we still increment by day but would need hour-level data
      'Hour': 1,
      'Weekly': 7,
      'Week': 7,
      'Monthly': 30,
