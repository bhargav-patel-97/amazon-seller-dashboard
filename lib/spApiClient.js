...
class SPAPIClient {
  ...
  async getSalesData(startDate, endDate, granularity = 'Daily') {
    ...
    try {
      ...
      const endISO = this.toISO8601WithTimezone(endDate, true); // End date should be exclusive (next day)
      
      const params = new URLSearchParams({
        marketplaceIds: this.config.marketplaceId,
        interval: `${startISO}--${endISO}`, // SP-API requires double dash format
        granularity: granularity === 'Daily' ? 'Day' : granularity, // SP-API uses 'Day' not 'Daily'
  ...
  toISO8601WithTimezone(dateString, endOfDay = false) {
    ...
    if (endOfDay) {
      // For end date, set to next day at 00:00:00 (exclusive)
    ...
    return date.toISOString(); // Returns format like "2025-09-22T00:00:00.000Z"
  }
